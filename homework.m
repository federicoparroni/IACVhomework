...
% The equation of an ellipse can be expressed from its properties:
%%
%
% $$ ax^2 + by^2 + bxy + dx + ey + f = 0 $$
%
% $$ if we know that the ellipse is center in (Xc,Yc), with axis a and b and
% orientation \theta, where: $$
%%
%
...
% Get the 4 tangent lines with different colors
...
%viewing_ray_stem = M \ stem_center;
...
figure();
hold on;
plotPoint3([0,0,0],'ko');  % center
% versors
plotLine3([0,0,0],[0.5,0,0],'r',1);
plotLine3([0,0,0],[0,0.5,0],'g',1);
plotLine3([0,0,0],[0,0,0.5],'b',1);
% viewpoint
plotPoint3(O,'bp');
% camera frame origin
plotPoint3(-t,'kh');
...
drawVRay(M,O,light_left, 'k');
drawVRay(M,O,light_right, 'k');

drawVRay(M,O,light_left, 'r');
drawVRay(M,O,light_right, 'r');

%drawVRay(M,O,stem_center, 'r');
...
% Now we fix the world reference frame equal to the camera
% reference. So: P = [K 0]
% P = [K [0;0;0]];
% O = null(P);
