...
We want to fix the world frame $W^1$ such that it lies in the car symmetry plane $\Pi_{sym}$ (we know that this plane is vertical and also orthogonal to $\Pi_p$, so it has equation $x=k$). So, we can intersect two viewing rays of a pair of symmetric features and then take the midpoint as the origin $O_{W^1}$ of the new frame:
...
\frac{x_1 + x_2}{2} = x_s & \quad P_1 \text{ and } P_1 \text{ are symmetric w.r.t } x=x_s \\
\end{cases} $$
...
\\
We can repeat this process multiple times for other pairs of symmetric points visible in the image.
\\
...
0 & 0 & 0 & 1
\end{pmatrix} $$
where $O_{W^1}$ is the origin of $W^1$ expressed in the coordinates of the frame $W^0$ (notice that the two reference frames have the same orientation by construction).

\subsection{Localize the camera relative to the car reference}
In the last section, when we have fixed the origin of the world frame, we have also established the translation between the reference frames of the camera and the world. In fact, at first, the two frame were coincident, so $t=(0, 0, 0)$. Then, the world frame has been set to a specific point $O=(x_0, y_0, z_0)$. This corresponds to a translation vector from the camera frame to the new world frame:
$$ t_{camera \rightarrow world} = (x_0, \: y_0, \: z_0) $$
Now, if we want to know the translation vector from the world to the camera, we just need to invert the previous relation:
$$ t_{world \rightarrow camera} = -t_{camera \rightarrow world} = (-x_0, \: -y_0, \: -z_0) $$
The rotation matrix $R$ will be:
$$ R_{world \rightarrow camera} = R^{-1}_{camera \rightarrow world} $$
...
$$ \begin{cases}
\frac{x_1-x_0}{l_1} = \frac{y_1-y_0}{m_1} = \frac{z_1-z_0}{n_1} \\
\frac{x_2-x_0}{l_2} = \frac{y_2-y_0}{m_2} = \frac{z_2-z_0}{n_2} \\
y_1 = y_2 \\
z_1 = z_2 
\end{cases} $$
where $d_1 = (l_1, m_1, n_1)$ and $d_2 = (l_2, m_2, n_2)$ are the viewing rays
...
We can now build the matrix that allows to compute the change of coordinates:
$$M_{world \leftarrow camera} = $$
